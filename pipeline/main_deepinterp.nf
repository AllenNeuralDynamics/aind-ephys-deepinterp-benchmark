...
// DeepInterpolation benchmark: baseline (lossless) vs DeepInterpolation-denoised,
// both followed by preprocessing + spike sorting, then hybrid ground-truth evaluation.
// Mirrors main_lossy.nf, swapping the WavPack-compression cases for a single
// `deepinterp` case (DeepInterpolation runs on the raw hybrid recording, before
// preprocessing, matching how the model was trained).
...
include { deepinterp } from './processes_spike_sorting_cases.nf'
...
if (!params.containsKey('deepinterp_args')) {
    deepinterp_args = ""
}
else {
    deepinterp_args = params.deepinterp_args
}
...
if (!params.containsKey("sorting_cases")){
    params.sorting_cases = "lossless+deepinterp"
}
else {
...
workflow {
    ...
    if ('deepinterp' in sorting_cases_list) {
        deepinterp_output_ch = deepinterp(
            max_duration_minutes,
            ecephys_ch.collect(),
            hybrid_generation_out.recordings.flatten(),
            params.sorter,
        ...
        sorter_results_ch = sorter_results_ch.mix(deepinterp_output_ch)
    }
